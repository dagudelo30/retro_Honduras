...
run_cpt=function(x,y,run,output,modes_x,modes_y,modes_cca,trans,type_trans){
  ...
  pred_y=str_split(basename(output),"_")[[1]][1]
  i_for=which(str_split(pred_y,"-")[[1]][1]==month.abb)
  len_se=length(str_split(pred_y,"-")[[1]])
  ...
  cmd<-gsub("%for_season%",i_for,cmd)
  cmd<-gsub("%len_season%",len_se,cmd)
...
lapply(folders,function(x) dir.create(paste0(x,"/output/sampling/sst_ersst"),recursive = T))
...
folder_output_r <- Map(function(x,y) paste0(x,"/output/sampling/sst_ersst/",substr(y,1,nchar(y))),folders,best_decil)
...
path_x_r <- lapply(folders,function(x)list.files(paste0(x,"/output/sampling/sst_ersst"),full.names = T))
...
col_names=c("response","predictor","decil","iteracion","gi")
